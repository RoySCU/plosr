...
RoutingProtocol::MprComputation ()
{
  ...
  for (NeighborSet::const_iterator neighbor = m_state.GetNeighbors ().begin ();
       neighbor != m_state.GetNeighbors ().end (); neighbor++)
    {
      ...
      {
        //超过200 则存入另一个邻居集
        if(NextPositionDistanse(*neighbor)){
          N_FAR.push_back (*neighbor);
        }
          //std::cout<<"distance: "<<NextPositionDistanse(*neighbor)<<std::endl;
      ...
      if (foundEntry)
        {
          ...
          //if(Ipv4Address("10.0.0.1") == m_mainAddress){
            ...
            if(found_2hop_Entry){
              //找到邻居表项
              NeighborTuple* last_neighbor = m_state.FindNeighborTuple(last_entry.nextAddr);
              //如果距离更小 则替换
              if(NextPositionDistanse(*last_neighbor)>NextPositionDistanse(*neighbor)){
              ...
              //<<"nextAddr:"<<last_entry.nextAddr<<"distance:"<<NextPositionDistanse(*last_neighbor));
              //NS_LOG_UNCOND("twoHopNeighborAddr:"<<nb2hop_tuple.twoHopNeighborAddr
              //<<"nextAddr:"<<entry.nextAddr<<"distance:"<<NextPositionDistanse(*neighbor));
            }
            //如果原本不存在entry
            else{
              //直接添加
