...
#define HELLO_POSITION_SIZE 4

namespace ns3 {
  
NS_LOG_COMPONENT_DEFINE ("POlsrHeader");
...
{
  static TypeId tid = TypeId ("ns3::polsr::PacketHeader")
    .SetParent<Header> ()
    .SetGroupName ("Olsr")
...
{
  static TypeId tid = TypeId ("ns3::polsr::MessageHeader")
    .SetParent<Header> ()
    .SetGroupName ("Olsr")
...
MessageHeader::Hello::Serialize (Buffer::Iterator start) const
{
  ...
  //加入gps等信息
  i.WriteHtonU16 (this->gps_x);
  i.WriteHtonU16 (this->gps_y);
  i.WriteHtonU32 (this->speed_x);
  i.WriteHtonU32 (this->speed_y);
  for (std::vector<LinkMessage>::const_iterator iter = this->linkMessages.begin ();
       iter != this->linkMessages.end (); iter++)
    {
      const LinkMessage &lm = *iter;
...
MessageHeader::Hello::Deserialize(Buffer::Iterator start, uint32_t messageSize)
{
  ...
  NS_ASSERT (messageSize >= 4+2*2+4*2);
  ...
  gps_x = i.ReadNtohU16 ();
  gps_y = i.ReadNtohU16 ();
  speed_x = i.ReadNtohU32 ();
  speed_y = i.ReadNtohU32 ();
  helloSizeLeft -= 2*2+4*2;
