...
#include "polsr-routing-protocol.h"
...
NS_LOG_COMPONENT_DEFINE ("POlsrRoutingProtocol");
...
{
  static TypeId tid = TypeId ("ns3::polsr::RoutingProtocol")
    ...
    .AddAttribute ("HnaInterval", "HNA messages emission interval.  Normally it is equal to TcInterval.",
                   ...
                   MakeTimeChecker ())
    .AddAttribute ("PosInterval", "position service interval.",
                   TimeValue (Seconds (0.5)),
                   MakeTimeAccessor (&RoutingProtocol::m_posInterval),
                   MakeTimeChecker ())
...
RoutingProtocol::SetIpv4 (Ptr<Ipv4> ipv4)
{
  NS_ASSERT (ipv4 != 0);
  NS_ASSERT (m_ipv4 == 0);
  NS_LOG_DEBUG ("Created polsr::RoutingProtocol");
...
RoutingProtocol::RecvOlsr (Ptr<Socket> socket)
{
  ...
  polsr::PacketHeader olsrPacketHeader;
      ...
      if (duplicated == NULL)
        {
          switch (messageHeader.GetMessageType ())
            {
            case polsr::MessageHeader::HELLO_MESSAGE:
            ...
            case polsr::MessageHeader::TC_MESSAGE:
              NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                            << "s OLSR node " << m_mainAddress
      ...
      if (do_forwarding)
        {
          ...
          if (messageHeader.GetMessageType ()  != polsr::MessageHeader::HELLO_MESSAGE)
            {
...
RoutingProtocol::RoutingTableComputation ()
{
  ...
  /*if(Ipv4Address("10.0.0.1") == m_mainAddress){
    NS_LOG_UNCOND ("Node " << m_mainAddress << ": RoutingTableComputation end.");
      ...
      if (foundEntry)
        {
          ...
          //找到邻居表项
          //NeighborTuple* neighbor = m_state.FindNeighborTuple(entry.nextAddr);
          //if(Ipv4Address("10.0.0.1") == m_mainAddress){
            RoutingTableEntry last_entry;
            bool found_2hop_Entry = Lookup (nb2hop_tuple.twoHopNeighborAddr, last_entry);
            //如果原本存在entry
            if(found_2hop_Entry){
              //如果距离更小 则替换
              ...
              //<<"nextAddr:"<<entry.nextAddr<<"distance:"<<NextPositionDistance(*neighbor));
            }
            //如果原本不存在entry
            else{
              //直接添加
...
///
void
RoutingProtocol::ProcessHello (const polsr::MessageHeader &msg,
                               const Ipv4Address &receiverIface,
                               const Ipv4Address &senderIface)
{
  NS_LOG_FUNCTION (msg << receiverIface << senderIface);

  const polsr::MessageHeader::Hello &hello = msg.GetHello ();
...
///
void
RoutingProtocol::ProcessTc (const polsr::MessageHeader &msg,
                            const Ipv4Address &senderIface)
{
  const polsr::MessageHeader::Tc &tc = msg.GetTc ();
...
///
void
RoutingProtocol::ProcessMid (const polsr::MessageHeader &msg,
                             const Ipv4Address &senderIface)
{
  const polsr::MessageHeader::Mid &mid = msg.GetMid ();
  Time now = Simulator::Now ();
...
///
void
RoutingProtocol::ProcessHna (const polsr::MessageHeader &msg,
                             const Ipv4Address &senderIface)
{

  const polsr::MessageHeader::Hna &hna = msg.GetHna ();
  Time now = Simulator::Now ();
  ...
  for (std::vector<polsr::MessageHeader::Hna::Association>::const_iterator it = hna.associations.begin ();
...
///
void
RoutingProtocol::ForwardDefault (polsr::MessageHeader olsrMessage,
...
RoutingProtocol::SendQueuedMessages ()
{
  ...
  for (std::vector<polsr::MessageHeader>::const_iterator message = m_queuedMessages.begin ();
...
RoutingProtocol::SendHello ()
{
  ...
  polsr::MessageHeader::Hello &hello = msg.GetHello ();
  ...
  for (LinkSet::const_iterator link_tuple = links.begin ();
       link_tuple != links.end (); link_tuple++)
    {
      ...
      polsr::MessageHeader::Hello::LinkMessage linkMessage;
      ...
      //设置链路距离
      linkMessage.setDistance(link_tuple->distance);
...
RoutingProtocol::SendTc ()
{
  ...
  polsr::MessageHeader::Tc &tc = msg.GetTc ();
  tc.ansn = m_ansn;
...
{
  polsr::MessageHeader msg;
  polsr::MessageHeader::Mid &mid = msg.GetMid ();
...
RoutingProtocol::SendHna ()
{
  ...
  msg.SetHopCount (0);
  msg.SetMessageSequenceNumber (GetMessageSequenceNumber ());
  polsr::MessageHeader::Hna &hna = msg.GetHna ();

  std::vector<polsr::MessageHeader::Hna::Association> &associations = hna.associations;
  ...
  for (Associations::const_iterator it = localHnaAssociations.begin ();
    ...
    {
      polsr::MessageHeader::Hna::Association assoc = { it->networkAddr, it->netmask};
...
///        local HNA associations to be sent by the node via HNA messages.
///        If this tuple does not exist, nothing is done (see "POlsrState::EraseAssociation()").
///
void
...
void
RoutingProtocol::LinkSensing (const polsr::MessageHeader &msg,
                              const polsr::MessageHeader::Hello &hello,
                              const Ipv4Address &receiverIface,
                              const Ipv4Address &senderIface)
{
  Time now = Simulator::Now ();
  ...
  if (link_tuple == NULL)
    {
      ...
      newLinkTuple.distance = NextPositionDistance(pos_t);
      //链路距离 end
  ...
  for (std::vector<polsr::MessageHeader::Hello::LinkMessage>::const_iterator linkMessage =
  ...
  if (updated)
    {
      //测试一下
      //if(Ipv4Address("10.0.0.1") == senderIface && Ipv4Address("10.0.0.2") == receiverIface)
      /*{
        //std::cout<<"now  -->";
        Vector item_pos = hello.GetPosition();
        Vector item_speed = hello.GetSpeed();
        double item_next_x = item_pos.x+item_speed.x*m_helloInterval.GetSeconds();
        double item_next_y = item_pos.y+item_speed.y*m_helloInterval.GetSeconds();
       //std::cout<<"next -->pos x:"<<item_next_x<<" y:"<<item_next_y<<std::endl;
        ...
        //std::cout<<"my now  -->"<<" x:"<<my_pos.x<<" y:"<<my_pos.y<<std::endl;
        //::cout<<"my next -->"<<" x:"<<my_next_x<<" y:"<<my_next_y<<std::endl;
        
        //现在的
        double d_x = my_pos.x-item_pos.x;
        double d_y = my_pos.y-item_pos.y;
        double d = sqrt(d_x*d_x+d_y*d_y);
        //std::cout<<"d now: "<<d<<std::endl;
        static double t_last=0,d_predict = 0;
        if(Simulator::Now ().GetSeconds()-t_last>3){
          std::cout<<"TIME:\t"<<Simulator::Now ().GetSeconds ()<<std::endl;
          std::cout<<"d now: "<<d<<" predict: "<<d_predict<<"IT IS OUT RANGE"<<std::endl;
        }
        t_last = Simulator::Now ().GetSeconds();
        //预测下一次
        double d_n_x = my_next_x-item_next_x;
        double d_n_y = my_next_y-item_next_y;
        double d_n = sqrt(d_n_x*d_n_x+d_n_y*d_n_y);
        double v_predict=d_n-d;
        d_predict = d_n;
        if(d_n > 200 && v_predict>0){
          //std::cout<<"TIME:\t"<<Simulator::Now ().GetSeconds ()<<std::endl;
          //std::cout<<"d next: "<<d_n
          //<<"v_predict:"<<v_predict
          //<<"\tWILL BE OUT RANGE"<<std::endl;
          LinkTuple *tuple = m_state.FindLinkTuple (senderIface);
          RemoveLinkTuple (*tuple);
        }
        else if(d_n + (d_n-d) >200){
          //std::cout<<"TIME:\t"<<Simulator::Now ().GetSeconds ()<<std::endl;
          //std::cout<<"d next: "<<d_n
          //<<"v_predict:"<<v_predict
          //<<"\tCould BE OUT RANGE"<<std::endl;
        }
        else{
            PositionTuple pos_t;
            pos_t.gps_x = item_pos.x;
            ...
            pos_t.speed_x = item_speed.x;
            pos_t.speed_y = item_speed.y;
            LinkTupleUpdated (*link_tuple, hello.willingness,pos_t);
        }
      }*/
      {
        Vector item_pos = hello.GetPosition();
  ...
  if (created)
    {
      ...
      pos_t.speed_x = item_speed.x;
      pos_t.speed_y = item_speed.y;
      LinkTupleAdded (*link_tuple, hello.willingness,pos_t);
...
void
RoutingProtocol::PopulateNeighborSet (const polsr::MessageHeader &msg,
                                      const polsr::MessageHeader::Hello &hello)
{
...
void
RoutingProtocol::PopulateTwoHopNeighborSet (const polsr::MessageHeader &msg,
                                            const polsr::MessageHeader::Hello &hello)
{
  Time now = Simulator::Now ();
  ...
  for (LinkSet::const_iterator link_tuple = m_state.GetLinks ().begin ();
       link_tuple != m_state.GetLinks ().end (); link_tuple++)
    {
      ...
      typedef std::vector<polsr::MessageHeader::Hello::LinkMessage> LinkMessageVec;
                  ...
                  if (nb2hop_tuple == NULL)
                    {
                      ...
                      //2跳节点距离
                      new_nb2hop_tuple.distance = linkMessage->distance+NextPositionDistance(hello);
                    ...
                    {
                      //2跳节点距离
                      nb2hop_tuple->distance = linkMessage->distance+NextPositionDistance(hello);
...
/// a new received HELLO message (following \RFC{3626}).
void
RoutingProtocol::PopulateMprSelectorSet (const polsr::MessageHeader &msg,
                                         const polsr::MessageHeader::Hello &hello)
{
  NS_LOG_FUNCTION (this);
...
RoutingProtocol::NeighborLoss (const LinkTuple &tuple)
{
  ...
  pos_t.speed_y =1000;
  pos_t.speed_x =1000;
  LinkTupleUpdated (tuple, OLSR_WILL_DEFAULT,pos_t);
...
void
RoutingProtocol::LinkTupleAdded (const LinkTuple &tuple, uint8_t willingness,const PositionTuple &pos )
{
  ...
  nb_tuple.neighborPosition = pos;
  nb_tuple.distance = NextPositionDistance(pos);
...
///
void
RoutingProtocol::LinkTupleUpdated (const LinkTuple &tuple, uint8_t willingness,const PositionTuple &pos)
{
  ...
  if (nb_tuple == NULL)
    {
      LinkTupleAdded (tuple, willingness,pos);
                           ...
                           uint16_t gps_distance)
{
  ...
  entry.gps_distance = gps_distance;
}
      ...
      for (uint32_t j = 0; j < m_ipv4->GetNAddresses (i); j++)
        {
          if (m_ipv4->GetAddress (i,j).GetLocal () == interfaceAddress)
            {
              AddEntry (dest, next, i, distance,gps_distance);
              return;
            }
        }
    }
  NS_ASSERT (false); // should not be reached
  AddEntry (dest, next, 0, distance,gps_distance);
}
...
Vector RoutingProtocol::GetPosition(Ipv4Address adr)
{
